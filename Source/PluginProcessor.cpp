...
void AmpPulserAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    mPulserLfoSliderValue = 3.0;
    mAngleDelta = 0.0f;
    mCurrentAngle[0] = 0.0f;
    mCurrentAngle[1] = juce::MathConstants<double>::pi;
}
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for(int i= 0; i<buffer.getNumSamples(); i++)
        {
            ...
            if(mPulserLfoSliderValue == 0.0)
                ...
                lfoSignal = std::cos(mCurrentAngle[channel]);
            ...
            // imcrement and wrap the main mCurrentAngle
            mCurrentAngle[channel] += mAngleDelta;
            mCurrentAngle[channel] = std::fmod(mCurrentAngle[channel], juce::MathConstants<double>::twoPi);
        }
    }
}
